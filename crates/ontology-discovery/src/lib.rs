...
pub fn discover_workspace(
    ...
    let universe = insert_node(
        &mut graph,
        None,
        OntologyType::Universe,
        NodeId::new("universe"),
        true,
    )?;
...
fn discover_ecosystem(
    ...
    for child in read_dir_sorted(ecosystem_dir)?
        .into_iter()
        .filter(|p| is_real_dir(p))
    {
        ...
        observations.push(DiscoveryObservation {
            path: rel(workspace, &child),
            kind: "project".into(),
            language: None,
            evidence: "ecosystem child directory".into(),
        });
        for repo in read_dir_sorted(&child)?
            .into_iter()
            .filter(|p| is_real_dir(p) && is_repository(p))
        {
...
fn discover_repository(
    ...
    insert_node(
        graph,
        Some(parent.clone()),
        OntologyType::Repository,
        repo_id.clone(),
        true,
    )?;
    let language = detect_language(repo_dir);
    observations.push(DiscoveryObservation {
        path: rel(workspace, repo_dir),
        kind: "repository".into(),
        language: language.clone(),
        evidence: "git or native manifest".into(),
    });

    let source_dirs = read_dir_sorted(repo_dir)?
        .into_iter()
...
fn discover_rust_semantics(
    ...
    let source = fs::read_to_string(path).map_err(|source| DiscoveryError::ReadSource {
            ...
            for (ordinal, item) in items.iter().enumerate() {
                ...
                let semantic_local = if item.name.is_some() {
                    if *occurrence == 1 {
                        ...
                        format!("ast:{base_key}:~{occurrence}")
                    }
                } else {
                ...
                observations.push(DiscoveryObservation {
                    path: rel(workspace, path),
                    kind: "rust-ast-item".into(),
                    language: Some("rust".into()),
                    evidence: format!(
                        "native_kind={native_kind}; ontology_type={}",
...
fn contains_source_files_recursive(
    path: &Path,
    ...
    fn walk(path: &Path, depth: usize, max_depth: Option<usize>) -> Result<bool, DiscoveryError> {
        ...
        for child in read_dir_sorted(path)? {
            ...
            if is_source_file(&child) {
                return Ok(true);
            }
            if child.is_dir() && walk(&child, depth + 1, max_depth)? {
                return Ok(true);
            }
        }
        Ok(false)
...
fn is_ecosystem_dir(path: &Path) -> bool {
    path.is_dir() && file_name(path, "").starts_with("ecosystem-")
}
fn is_real_dir(path: &Path) -> bool {
    path.is_dir() && !is_ignored(path) && !file_name(path, "").starts_with('.')
}
fn is_ignored(path: &Path) -> bool {
    IGNORED.contains(&file_name(path, "").as_str())
}
...
fn detect_language(repo: &Path) -> Option<String> {
    ...
    if has("Cargo.toml") {
        Some("rust".into())
    } else if has("package.json") || has("pnpm-workspace.yaml") {
        Some("node".into())
    } else if has("go.mod") {
        Some("go".into())
        ...
        Some("java".into())
    } else if read_dir_sorted(repo).ok().is_some_and(|items| {
        items
            .iter()
            .any(|p| p.extension().and_then(|v| v.to_str()) == Some("kt"))
    }) {
        Some("kotlin".into())
    } else {
        None
...
mod tests {
    ...
    fn fixture() -> PathBuf {
        ...
        fs::write(
            root.join("ecosystem-demo/project-a/repo-a/Cargo.toml"),
            "[package]\nname='repo-a'\nversion='0.1.0'\n",
        )
        .unwrap();
        fs::write(
            root.join("ecosystem-demo/project-a/repo-a/src/core/lib.rs"),
            "pub fn demo() {}\n",
        )
        .unwrap();
        root
    ...
    fn discovery_does_not_fabricate_unit_or_module() {
        ...
        assert!(result
            .observations
            .iter()
            .any(|x| x.kind == "unmaterialized-levels"));
    ...
    fn discovers_nested_rust_source_and_execution_observation() {
        ...
        let execution = result
            .graph
            .nodes_by_type(OntologyType::Execution)
            .next()
            .unwrap();
        assert_eq!(execution.parent, None);
        assert!(result
            .graph
            .outgoing_kind(&execution.id, EdgeKind::ObservedAt)
            .next()
            .is_some());
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn repository_directly_under_ecosystem_is_not_forced_into_fake_project() {
        ...
        fs::write(
            root.join("ecosystem-demo/repo-a/Cargo.toml"),
            "[package]\nname='repo-a'\nversion='0.1.0'\n",
        )
        .unwrap();
        fs::write(
            root.join("ecosystem-demo/repo-a/src/lib.rs"),
            "pub fn demo() {}\n",
        )
        .unwrap();
        let result = discover_workspace(&root, registry(), DiscoveryOptions::default()).unwrap();
