...
fn normalized_line(line: &str) -> String {
    ...
    loop {
        let next = current
            ...
            .or_else(|| current.strip_prefix("final "))
            .or_else(|| current.strip_prefix("open "))
            .or_else(|| current.strip_prefix("internal "))
...
fn js_rules(line: &str) -> Vec<(OntologyType, &'static str, Option<String>, &'static str)> {
    ...
    if let Some(n) = after_keyword(line, "class") {
        out.push((OntologyType::Entity, "class", Some(n), "class declaration"));
    }
    if let Some(n) = after_keyword(line, "interface") {
        out.push((
            OntologyType::Entity,
            "interface",
            Some(n),
    ...
    if let Some(n) = after_keyword(line, "type") {
        out.push((
            OntologyType::Entity,
            "type_alias",
            Some(n),
    ...
    if let Some(n) = after_keyword(line, "function") {
        out.push((
    ...
    if let Some(n) = assignment_function(line) {
        out.push((
            OntologyType::Function,
            "arrow_function",
            Some(n),
    ...
    if let Some(n) = variable_name(line) {
        out.push((
            OntologyType::Value,
            "variable",
            Some(n),
...
fn python_rules(line: &str) -> Vec<(OntologyType, &'static str, Option<String>, &'static str)> {
    ...
    if let Some(rest) = line.strip_prefix("async ") {
        if let Some(n) = after_keyword(rest, "def") {
            out.push((
                OntologyType::Function,
                "async_function",
                Some(n),
    ...
    if let Some(n) = assignment_name(&line) {
        out.push((OntologyType::Value, "assignment", Some(n), "assignment"));
    }
    out
}
...
fn go_rules(line: &str) -> Vec<(OntologyType, &'static str, Option<String>, &'static str)> {
    ...
    let line = normalized_line(line);
    if let Some(n) = after_keyword(&line, "type") {
        out.push((OntologyType::Entity, "type", Some(n), "type declaration"));
    }
    if let Some(n) = go_function_name(&line) {
        out.push((
            OntologyType::Function,
            "function",
            Some(n),
    ...
    if line.starts_with("import ") || line.starts_with("const ") || line.starts_with("var ") {
        out.push((
            OntologyType::Instruction,
            "declaration",
            None,
            "package/import/declaration statement",
        ));
    }
    out
}
...
fn jvm_rules(
    line: &str,
    language: Language,
) -> Vec<(OntologyType, &'static str, Option<String>, &'static str)> {
    let mut out = Vec::new();
    let line = normalized_line(line);
    for keyword in ["class", "interface", "object", "enum class"] {
        if let Some(n) = after_keyword(&line, keyword) {
            out.push((OntologyType::Entity, keyword, Some(n), "type declaration"));
        }
    }
    if let Some(n) = after_keyword(
        &line,
...
mod tests {
    ...
    fn javascript_adapter_finds_core_declarations() {
        ...
        assert!(items
            .iter()
            .any(|x| x.native_kind == "class" && x.name.as_deref() == Some("User")));
        assert!(items
            .iter()
            .any(|x| x.native_kind == "function" && x.name.as_deref() == Some("load")));
        assert!(items
            .iter()
            .any(|x| x.ontology_type == OntologyType::Instruction));
    }
    ...
    fn python_adapter_finds_definitions() {
        ...
        assert_eq!(
            items
                .iter()
                .filter(|x| x.ontology_type == OntologyType::Entity)
                .count(),
            1
        );
        assert!(items
            .iter()
            .any(|x| x.native_kind == "async_function" && x.name.as_deref() == Some("load")));
        assert!(items
            .iter()
            .any(|x| x.ontology_type == OntologyType::Function
                && x.name.as_deref() == Some("helper")));
    }
    ...
    fn go_adapter_finds_receiver_functions() {
        ...
        assert!(items.iter().any(
            |x| x.ontology_type == OntologyType::Function && x.name.as_deref() == Some("Load")
        ));
    }
    ...
    fn java_adapter_finds_non_void_methods() {
        ...
        assert!(items
            .iter()
            .any(|x| x.native_kind == "method" && x.name.as_deref() == Some("load")));
    }
